        ...
        <div class="col">
          ...
          <form action="{{ route('mahasiswas.store') }}" method="POST">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('major') is-invalid @enderror" id="major" name="major" value="{{ old('major') ?? $mahasiswa->major}}">
              @error('major')
                <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
            <button type="submit" class="btn btn-primary mt-2">Edit Mahasiswa</button>
        </form>
        </div>
      </div>
    </div>
